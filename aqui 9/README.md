...
assets/categories/  category tile photos, one per category id
```

## Category tile photos

The browse tiles show a photo with the category name below it. Each tile looks
for a file in `assets/categories/` named after the category id in
`data/categories.js`, for example `assets/categories/skincare.jpg`. Either
`.jpg` or `.png` works.

If no file is there, that tile falls back to the line icon on the paper ground,
so photos can arrive one at a time and a new category never needs a photo to
work. Replacing a photo means overwriting the file. Nothing in the code
changes either way.

Square images, 1000 x 1000 pixels or larger, cropped consistently, keep the
grid reading as one set. The files in `assets/categories/` today are
placeholders cut from the home screen photo, there to show the layout until the
real category photos arrive.
